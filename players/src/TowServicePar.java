...
public class TowServicePar {
    ...
    public TowServicePar() {
        roadProperties = ROADProperties.getInstance("players.properties");
//        opRateLimiters.put("tow", new OperationRateLimiter("TowServicePar","tow", roadProperties));
        opRateLimiters.put("pickUp", new OperationRateLimiter("TowServicePar", "pickUp", roadProperties));
        opRateLimiters.put("deliver", new OperationRateLimiter("TowServicePar", "deliver", roadProperties));
    }
    ...
    public TowReturn tow(String pickupLocation, String garageLocation) throws AxisFault {
        ...
        if (log.isInfoEnabled()) {
            log.info("Tow in TowServicePar received >>>>>>>>> : " + pickupLocation
                    + " :::::: " + garageLocation);
        }
    ...
    public String pickUp(String pickupLocation) throws AxisFault {
        ...
        if (log.isInfoEnabled()) {
            log.info("PickUp in TowServicePar received >>>>>>>>> : " + pickupLocation);
        }
        if (!rateLimier.tryConsume()) {
    ...
    public String deliver(String garageLocation) throws AxisFault {
        ...
        if (log.isInfoEnabled()) {
            log.info("Deliver in TowServicePar received >>>>>>>>> : " + garageLocation);
        }
        if (!rateLimier.tryConsume()) {
            String msg = "Capacity limit has reached for tow : " + rateLimier.getThreshold();
            log.error(msg);
    ...
    public void payTow(String content) {
        if (log.isInfoEnabled()) {
            log.info("payTow in TowServicePar received >>>>>>>>> : " + content);
            log.debug("TC paid for towing service");
        }
    }

    public void setInsuranceFacts(PFacts facts) {
        log.info("TowServicePar no of facts received = " + facts.getFact().length);
