...
//        this.schema_namespace_prefix = service.getSchemaTargetNamespacePrefix();
    ...
    public void buildInMessage(AxisOperation axisOperation,
                               List<au.edu.swin.ict.road.common.Parameter> facts) {
        try {
            ...
            XmlSchemaSequence sequence = generateSchema(facts, partQName, "parameter");
...
//            setParentElementQName(facts, elementQName);
     ...
     *
     * @param axisOperation <code>AxisOperation</code> instance
     * @param returnType    A list of results that defines the arguments of the WSDL out message
     */
    public void buildOutMessage(AxisOperation axisOperation,
        ...
        try {
            ...
            List<au.edu.swin.ict.road.common.Parameter> returnTypes =
                    new ArrayList<au.edu.swin.ict.road.common.Parameter>();
            returnTypes.add(new au.edu.swin.ict.road.common.Parameter(returnType, "return"));
            XmlSchemaSequence sequence = generateSchema(returnTypes,
                    partQName, "item");
            XmlSchemaComplexType methodSchemaType =
                    createSchemaTypeForMethodPart(partQName);
            if (sequence != null) {
     ...
     */
    private XmlSchemaSequence generateSchema(List<au.edu.swin.ict.road.common.Parameter> descriptions,
        ...
        for (au.edu.swin.ict.road.common.Parameter description : descriptions) {
...
//                            name = firstChar + name.substring(1);
            ...
            XmlSchemaElement el = new XmlSchemaElement(xmlSchema, false);
            if (isSimpleType) {
    ...
    private void setSchemaTypeOnElement(XmlSchemaElement el, String className) {
        ...
        for (XmlSchema schema : xmlSchemas) {
            Iterator<?> iterator = schema.getItems().iterator();
    ...
    private void generateSchemaForClass(String className) {
        try {
            ...
            for (Object typeName : map.keySet()) {
                if (typeTable.getComplexSchemaType((String) typeName) == null) {
    ...
    private void mergeTwoSchemaTypes(Map<String, XmlSchema> xmlSchemaMap) {
        for (String ns : xmlSchemaMap.keySet()) {
            ...
            Iterator<?> iterator = tobeAdd.getItems().iterator();
            while (iterator.hasNext()) {
                ...
                boolean canAdd = true;
                List<XmlSchemaObject> collection = xmlSchema.getItems();
...
//            globalElement.setsetQName(elementName);
            xmlSchema.getItems().add(globalElement);
            xmlSchema.getElements().put(elementName, globalElement);
        }
        if (typeTable.getComplexSchemaType(localPartName) == null) {
    ...
    private XmlSchema getXmlSchema(String targetNamespace) {
        ...
        if (!schemaMap.containsKey(targetNamespace)) {
            ...
            return schemaMap.get(targetNamespace);
        }
    }
