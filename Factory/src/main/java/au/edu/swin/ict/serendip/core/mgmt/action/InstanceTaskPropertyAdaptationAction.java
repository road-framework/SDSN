 ...
 * @author Malinda
 */
public class InstanceTaskPropertyAdaptationAction implements InstanceAdaptAction, Constants {
    ...
    public InstanceTaskPropertyAdaptationAction(String taskId, String propertyId, String newVal) {
        this.taskId = taskId;
    ...
    public boolean adapt(ProcessInstance pi) throws AdaptationException {
        ...
        //if the task is currently is already completed, reject the property change
        if (task.getCurrentStatus() == Task.status.completed) {
            throw new AdaptationException("Task already completed. Cannot perform adaptation. Task=" + this.taskId);
        }
        //If the task is currently in progress and the property is change in eppre or performance property then reject
        if (task.getCurrentStatus() == Task.status.active) {
            if (this.proeprtyId.equals(Task.propertyAttribute.preep.toString()) || this.proeprtyId.equals(Task.propertyAttribute.pp.toString())) {
        ...
        if (this.proeprtyId.equals(Task.propertyAttribute.preep.toString())) {
            task.removeAllEventPatterns();
            task.addEventPattern(this.newVal);
        } else if (this.proeprtyId.equals(Task.propertyAttribute.postep.toString())) {
        ...
        } else if (this.proeprtyId.equals(Task.propertyAttribute.descr.toString())) {
            //
        } else {
            throw new AdaptationException("Unknown proeprty for task adaptation " + this.proeprtyId);
        }
        return true;
    }
