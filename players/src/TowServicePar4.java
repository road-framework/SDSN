...
public class TowServicePar4 {
    private static Logger log = Logger.getLogger(TowServicePar4.class.getName());
    ...
    public TowServicePar4() {
        roadProperties = ROADProperties.getInstance("players.properties");
//        opRateLimiters.put("tow", new OperationRateLimiter("TowServicePar4","tow", roadProperties));
        opRateLimiters.put("pickUp", new OperationRateLimiter("TowServicePar4", "pickUp", roadProperties));
        opRateLimiters.put("deliver", new OperationRateLimiter("TowServicePar4", "deliver", roadProperties));
    }
    ...
    public TowReturn tow(String pickupLocation, String garageLocation) throws AxisFault {
        ...
        if (log.isInfoEnabled()) {
            log.info("Tow in TowServicePar4 received >>>>>>>>> : " + pickupLocation
                    + " :::::: " + garageLocation);
        }
    ...
    public String pickUp(String pickupLocation) throws AxisFault {
        ...
        if (log.isInfoEnabled()) {
            log.info("PickUp in TowServicePar4 received >>>>>>>>> : " + pickupLocation);
        }
        if (!rateLimier.tryConsume()) {
    ...
    public String deliver(String garageLocation) throws AxisFault {
        ...
        if (log.isInfoEnabled()) {
            log.info("Deliver in TowServicePar4 received >>>>>>>>> : " + garageLocation);
        }
        if (!rateLimier.tryConsume()) {
            String msg = "Capacity limit has reached for tow : " + rateLimier.getThreshold();
            log.error(msg);
    ...
    public void payTow(String content) {
        if (log.isInfoEnabled()) {
            log.info("payTow in TowServicePar4 received >>>>>>>>> : " + content);
            log.debug("TC paid for towing service");
        }
    }

    public void setInsuranceFacts(PFacts facts) {
        log.info("TowServicePar4 no of facts received = " + facts.getFact().length);
