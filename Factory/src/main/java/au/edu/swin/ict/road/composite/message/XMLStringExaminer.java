...
/**
 * Class for checking the message preference and extracting the contents of
 * the message from an XML string.
 * <br/>
            ...
            if (this.contentType.equalsIgnoreCase("xmlstring")) {
                ...
                for (String query : queryNameList) {
                    ...
                    if (xmlString.contains(query)) {
                        // Retrieve the content of the query by splitting the tags
                        String value = xmlString.substring(xmlString.indexOf(query) + query.length() + 1, xmlString.lastIndexOf("</" + query));
