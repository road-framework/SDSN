...
public class ScriptConstructor {
    private static Logger log = Logger.getLogger(ScriptConstructor.class.getName());
    ...
    public static Script constructScript(String contents) throws Exception {
        try {
            ...
            if (null == tree) {
                throw new Exception("Cannot construct the abstract syntax tree.");
            }
//				showTree(tree);
//				printTree(tree, 2);
            ///
        ...
        for (int i = 0; i < t.getChildCount(); i++) {
            ...
            switch (child.getType()) {
                case behavLexer.BTNAME:
                    String btName = child.getFirstChildWithType(behavLexer.WORD).getText();
                    behav.setBehaviorId(btName);
                    break;
                case behavLexer.EXTENDS:
                    String extendId = child.getFirstChildWithType(behavLexer.WORD).getText();
        ...
        for (int i = 0; i < t.getChildCount(); i++) {
            ...
            switch (child.getType()) {
                case behavLexer.TASKNAME:
                    String taskId = child.getFirstChildWithType(behavLexer.WORD).getText();
                    task.setTaskId(taskId);
                    break;
                case behavLexer.PRE:
                    String pre = child.getFirstChildWithType(behavLexer.STRING).getText();
                    task.setPreEP(pre);
                    break;
                case behavLexer.POST:
                    String post = child.getFirstChildWithType(behavLexer.STRING).getText();
                    task.setPostEP(post);
                    break;
                case behavLexer.PP:
                    String pp = child.getFirstChildWithType(behavLexer.WORD).getText();
                    task.setPp(pp);
                    break;
                case behavLexer.ROLE:
                    String role = child.getFirstChildWithType(behavLexer.WORD).getText();
                    task.setRoblig(role);
                    break;
            }//switch
        }//for
