 ...
 * <p/>
 * Assumptions:
 * 1. A script can contain a number of commands seperated by ';'
 * 2. Each command can contain a finite number of arguments
 * 2. Always the first argument of a command is the pid
 * <p/>
 * Example Script:
 * PI:PDgold11{
 * updatePropertyOfTask tid="Tow" property="postep" value="eTowSuccess * eTXReqd";
 * <p/>
 * }
 *
 * @author Malinda
...
public class AdaptationScriptEngine implements Constants {
    ...
    public void execute(String scriptText) throws AdaptationException {
        ...
        //Lets admit a block by block
        List<Block> blocks = script.getBlocks();

        if (blocks.size() < 1) {
            throw new AdaptationException("Script error. No Scope? No parsable blocks? Script=" + scriptText);
        }
        for (Block b : blocks) {
            executeBlock(b);
        }
    ...
    /**
     * Executes a block. A block must have a scope and an id to identify the scope
     * e.g., PI:p001 a process instance
     * e.g., DEF:smc is an identification of the smc
     *
     * @param b
    ...
    private void executeBlock(Block b) throws AdaptationException {
        ...
        //Lets admit a command by command
        String[] blockNameSplit = blockName.split(":");
        if (blockNameSplit.length != 2) {
            throw new AdaptationException("Invalid block scope specification " + blockName);
        }
        ...
        if (blockScope.equals(scope.PI.toString())) {
            ...
            // Check if multiple process instances needs to be adapted. A comma separated string.
            String[] pids = null;
            ...
            //If there is any problem adapting a single instance, throw an exception
            for (String pid : pids) {
                //Collect all adaptation actions
                List<InstanceAdaptAction> adaptationActionsList = new ArrayList<InstanceAdaptAction>();
                for (Command c : cmds) {
                    InstanceAdaptAction act = this.getInstanceAdaptationAction(c, pid);
                    adaptationActionsList.add(act);
                }
                //admit the collected adaptation actions on the process instance pid
//                OrganiserMgtOpResult result = engine.getSerendipOrgenizer().adaptProcessInstance(pid, adaptationActionsList);
//                //If there is an error record it
//                if (false == result.getResult()) {
//                    throw new AdaptationException("Adaptation failed for " + pid + "Reason: " + result.getMessage());
//
//                }
            }
            ...
            //admit the collected adaptation actions
//            OrganiserMgtOpResult result = engine.getSerendipOrgenizer().adaptDefinition(adaptationActionsList);
//            if (false == result.getResult()) {
...
//            }
     ...
     * @throws AdaptationException
     */
    private InstanceAdaptAction getInstanceAdaptationAction(Command c, String pid) throws AdaptationException {
        ...
        } else if (commandName.equals(commands.updateProcessInstanceState.name())) {
            action = new InstanceProcessStateAdaptationAction(c.getProperty(KEY_STATUS));
        } else if (commandName.equals(commands.pause.name())) {
            action = new InstanceProcessStateAdaptationAction(ProcessInstance.status.paused.toString());
        } else if (commandName.equals(commands.resume.name())) {
            action = new InstanceProcessStateAdaptationAction(ProcessInstance.status.active.toString());
        } else {
            throw new AdaptationException("Unspported command " + commandName + " for process instance " + pid);
        }
     ...
     * @throws AdaptationException
     */
    private DefAdaptAction getDefAdaptationAction(Command c) throws AdaptationException {
        ...
        if (commandName.equals(commands.applyPatch.name())) {
            ...
            action = new DefAddBehaviorRefToProcessDef(c.getProperty(KEY_PDID), c.getProperty(KEY_BU_ID));
        } else if (commandName.equals(commands.expandTheRole.name())) {
            action = new CustomDefAdaptScaleOut(commands.expandTheRole.name(), c.getProperties());
        } else if (commandName.equals(commands.scaleInTheComposite.name())) {
            action = new CustomDefAdaptScaleIn(commands.scaleInTheComposite.name(), c.getProperties());
        } else {
    ...
    /**
     * This method loads an object of a custom adaptation action class.
     * Such a class should
     * <ul>
     * <li> Extend class interface <code> au.edu.swin.ict.serendip.core.mgmt.action.CustomDefAdaptationAction</code>
     * </ul>
     * The command should be like
     * [anyUserDefinedCommand] class=[fullQlfdClassName] (key=val)*
     *
     * @param c
     * @return
     * @throws AdaptationException
     */
    private DefAdaptAction loadCustomDefAdaptationAction(Command c) throws AdaptationException {
        CustomDefAdaptationAction action = null;
        String className = c.getProperty(KEY_CUSTOM_CLASS);
        if (null == className) {
            throw new AdaptationException("Unspported command OR the key  " + KEY_CUSTOM_CLASS + " is not specified for custom command " + c.getName());
        }
        try {
            Class<CustomDefAdaptationAction> clazz = (Class<CustomDefAdaptationAction>) Class.forName(className);

            Constructor<CustomDefAdaptationAction> ct = clazz.getConstructor(String.class, Properties.class);
            action = (CustomDefAdaptationAction) ct.newInstance(c.getName(), c.getProperties());//Here we set the command name and properties

        } catch (Exception e) {
            throw new AdaptationException("Cannot load class " + className + " due to error " + e.getMessage());
        }
        return action;
    }
