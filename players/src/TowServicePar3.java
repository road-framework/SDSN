...
public class TowServicePar3 {
    private static Logger log = Logger.getLogger(TowServicePar3.class.getName());
...
//        opRateLimiters.put("tow", new OperationRateLimiter("TowServicePar1","tow", roadProperties));
        opRateLimiters.put("pickUp", new OperationRateLimiter("TowServicePar3", "pickUp", roadProperties));
        opRateLimiters.put("deliver", new OperationRateLimiter("TowServicePar3", "deliver", roadProperties));
    }
    ...
    public TowReturn tow(String pickupLocation, String garageLocation) throws AxisFault {
        ...
        if (log.isInfoEnabled()) {
            log.info("Tow in TowServicePar3 received >>>>>>>>> : " + pickupLocation
                    + " :::::: " + garageLocation);
        }
    ...
    public String pickUp(String pickupLocation) throws AxisFault {
        ...
        if (log.isInfoEnabled()) {
            log.info("PickUp in TowServicePar3 received >>>>>>>>> : " + pickupLocation);
        }
        if (!rateLimier.tryConsume()) {
    ...
    public String deliver(String garageLocation) throws AxisFault {
        ...
        if (log.isInfoEnabled()) {
            log.info("Deliver in TowServicePar3 received >>>>>>>>> : " + garageLocation);
        }
        if (!rateLimier.tryConsume()) {
            String msg = "Capacity limit has reached for tow : " + rateLimier.getThreshold();
            log.error(msg);
    ...
    public void payTow(String content) {
        if (log.isInfoEnabled()) {
            log.info("payTow in TowServicePar3 received >>>>>>>>> : " + content);
            log.debug("TC paid for towing service");
        }
    }

    public void setInsuranceFacts(PFacts facts) {
        log.info("TowServicePar3 no of facts received = " + facts.getFact().length);
