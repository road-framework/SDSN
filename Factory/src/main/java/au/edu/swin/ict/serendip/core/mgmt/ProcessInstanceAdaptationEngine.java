...
public class ProcessInstanceAdaptationEngine {
    ...
    /**
     * This method should be called to perform a single or an array of adaptation
     *
     * @param pId            the process instance id
     * @param adaptationList contains a number of atomic adaptation actions.
     * @throws AdaptationException
     */
    public void executeAdaptation(String pId, List<InstanceAdaptAction> adaptationList) throws AdaptationException {
        //Get the process instance
        ProcessInstance pi = this.engine.getProcessInstanceByInsId(pId);
        //Get the task
        if (null == pi) {
            throw new AdaptationException("Cannot find the process isntance" + pId);
        }
        //Backup it first
        this.backup(pi);
        ...
        for (InstanceAdaptAction aa : adaptationList) {
            try {
                ...
                if (!res) {
                    this.restore();
                    throw new AdaptationException("Problem in adapting process instance " + pId);
                }
            } catch (AdaptationException e) {
                // TODO Auto-generated catch block
                e.printStackTrace();
                this.restore();
                throw new AdaptationException("Adaptation aborted due to following reason.\n" + e.getMessage());
            }
        }
        //Validate	against the constraints
        try {
            boolean isValid = this.engine.getModelFactory().verifyProcessInstance(pi);
            if (!isValid) {
                this.restore();
                throw new AdaptationException("Verification Failed. Adaptation aborted.\n");
            }
        } catch (SerendipVerificationException e) {
            // TODO Auto-generated catch block
            e.printStackTrace();
            this.restore();
            throw new AdaptationException("Adaptation invalid due to following reason.\n" + e.getMessage());
        }
