...
public class FactSynchroniser {
     ...
     *
     * @param factType the id of the <code>FactSynchroniser</code>
     */
    public FactSynchroniser(String factType, Role associatedRole,
     ...
     *
     * @param ctxClass the class name of the <code>IContextProvider</code>
     *                 implementation.
     */
     ...
     *
     * @param fact the <code>FactObject</code> with the desired structure.
     */
     ...
     * <code>DroolsContractRules</code> classes.
     */
     ...
     *
     * @param interaction the interaction style between the regulator and the external
     *                    entities.
     */
     ...
     * will be ignored.
     *
     * @param time the synchronisation interval in milliseconds.
     */
     ...
     *
     * @param facts XML String which contains the facts
    ...
    public boolean factsChanged(Collection<FactObject> FTSFacts) {
        boolean isSynchronizationDone = false;
    ...
    public void manageRemoveOperations(String factOp, String factId) {
        ...
        for (FactObject extFact : externalFacts) {
            if (extFact.getUniqueId().equals(factId)) {
                result = associatedRole.getComposite().getFTS().getFactTupleSpaceRow(factType).deleteFact(extFact);
            }
        }
        response += result;
        response += "</return>";
    ...
    public Collection<FactObject> getLatestFactsFromFTS() {
        FactTupleSpace FTSRef = associatedRole.getComposite().getFTS();
