...
public class OrgNewOperationTester {
    private static Logger log = Logger.getLogger(OrgNewOperationTester.class.getName());
...
//                                 parameters, termId, contractId);
        ...
        /** make sure we can send a message correctly using the new term and operation **/
        IRole waiter = smc.getRoleByID("wt");
        IRole chef = smc.getRoleByID("cf");

        // need to add a new permission rule to allow
//        or = org.addNewContractRule("rule \"test\" when $event : MessageReceivedEvent(operationName == \"test\") then $event.setBlocked(false); System.out.println(\"test message from waiter to chef\");end", contractId);
    ...
    public class WaiterPlayer implements Runnable {
        ...
        @Override
        public void run() {
            MessageWrapper req = new MessageWrapper("test message", "test", false);
    ...
    public class ChefPlayer implements Runnable {
        ...
        public void run() {
            ...
            log.debug("Chef: recieved message, op name: " + req.getOperationName());
            MessageWrapper resp = new MessageWrapper("test message response", "test", true);
