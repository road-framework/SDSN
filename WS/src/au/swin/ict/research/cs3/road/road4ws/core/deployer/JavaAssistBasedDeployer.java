...
/**
 * This is the implementation based on javassist where we try to deploy a new .class file as a POJO and then use Axis2's
 * existing support for POJO. Caused some issues related to role re-deployment, Hence currently not in use.
 */
public class JavaAssistBasedDeployer extends ROADDeployer {
    ...
    protected void unDeployARole(String compositeName, String roleName) {
        String fileName = this.getNoramlizedServiceName(compositeName, roleName);
        File rolesDir = new File(axisHome
                + ROADConstants.ROAD4WS_ROLE_FILES_DIR);
        for (File f : rolesDir.listFiles()) {
            String fName = f.getName().toLowerCase();
