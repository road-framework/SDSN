...
public class DefaultVerification implements SerendipVerification {
    private static Logger log = Logger.getLogger(DefaultVerification.class.getName());
    ...
    private VerificationResult verifyProcessDefinition(Composition comp,
        ...
        List<String> btRefIdList = pd.getCollaborationUnitRef();
        String[] btRefIdArr = new String[btRefIdList.size()];
        CollaborationUnitType[] btArr = CompositionUtil.getBehaviorTermsForBTIDs(
                btRefIdList.toArray(btRefIdArr), comp);
        for (int i = 0; i < btArr.length; i++) {
            List<TaskRefType> taskTypeList = btArr[i].getConfigurationDesign().getTaskRef();
