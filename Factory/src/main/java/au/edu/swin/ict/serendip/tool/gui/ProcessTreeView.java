...
//				"\n Check above check-box to view the completed process instances")
    ...
    private void createPINodes() {
        ...
        if (this.showOldChkBox.isSelected()) {
            processInstances = this.engineInstance.getCompletedProcessInstances();
        } else {
        ...
        Set<String> enumProcessIns = processInstances.keySet();
        for (String pid : enumProcessIns) {

            ProcessInstance pi = processInstances.get(pid);

            DefaultMutableTreeNode piNode = new DefaultMutableTreeNode(pi.getId());
            rootNode.add(piNode);
            List<BehaviorTerm> btVec = pi.getBtVec();
        ...
        // Set icon
        ImageIcon leafIcon = null;// createImageIcon("images/process_arrow.ico");// E:/ROAD/svn/serendip/trunk
        if (leafIcon != null) {
    ...
    private ConfigurableEPC markVisitedEventsAndTasks(ConfigurableEPC epc, ProcessInstance pi) {
        Collection<EventRecord> triggeredEvents = this.engineInstance.getEventCloud().
                getEventRecordsForPid(pi.getClassifier().getProcessInsId());
        epc = SerendipView.markFiredEvents(epc, triggeredEvents);
        epc = SerendipView.markExecutedTasks(epc, pi.getCompletedTasks().keySet(), pi.getCurrentTasks().keySet());
        return epc;
    }
    ...
    public void valueChanged(TreeSelectionEvent e) {
        ...
        if (nodes.length <= 2) {
            ProcessInstance pi = this.engineInstance
                    .getProcessInstanceByInsId(nodes[1].toString());
            ...
            try {
                if (null != pi) {
                    ...
                    SerendipEPCView epcView = new SerendipEPCView(
                            pi.getId(), showEPC);
                    this.epcViewScrol.setViewportView(epcView);
                } else {
                    System.err.println("ERROR: Process instance is null in Process Tree View");
                }
