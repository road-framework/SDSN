...
/**
 * Composite Marshalling is responsible for generating a complete updated snapshot
 * of the runtime composite with the updated SMC and rule files. It takes a
 * runtime Java Composite object and uses it to get its current state in-order
 * to create an XML representation and store it in a file. It creates all the
 * binding objects which were earlier created during XML to JAVA instantiation.
 * JAXB is being used as the library for Java to XML transformation. The
 * corresponding rule files are also updated according to the rule changes done
 * and saved along with the SMC. This snapshot is saved in the snapshots
 * directory and is arranged according to the current time-stamp in-order to
 * create a history of snapshots for the composite.
 *
...
public class CompositeMarshalling {
     ...
     * Sets the dirPath.
     *
     * @param dirPath the dirPath to be set
     */
     ...
     * Sets the foldername.
     *
     * @param foldername the foldername to be set
     */
     ...
     *
     * @param c the composite object which has to be converter to an XML
     *          representation
     */
     ...
     * @param c       the composite object which has to be converter to an XML
     *                representation
     * @param dirPath the String which represents the directory path for storing the
     *                XML representation
     */
    ...
    /**
     * Converts the forward time composite object to an XML representation and
    ...
    private boolean convertComposite(Composite c) {
        ...
        smcBinding.setServiceBindings(playerBindings);
        ...
        try {
            ...
            if (log.isInfoEnabled()) {
                log.info("Saving the SMC to an XML");
            }
        ...
        if (marshalDRL(c)) {
            if (log.isInfoEnabled()) {
                log.info("Snapshot Timestamp: " + this.getFoldername());
                log.info("Snapshot stored at: " + dirPath);
            }
            return true;
        } else {
    ...
    private boolean marshalDRL(Composite c) {
        ...
        while (contractIterator.hasNext()) {
            ...
            String droolFileName = tempContract.getRules().getRuleFile();
                        ...
                        if (drlFile.contains(fileList[i].getName())) {
                            ...
                            while ((tempLine = droolsFile.readLine()) != null) {
                                ...
                                if (tempLine.contains(ruleName)) {
                                    ...
                                    String endString[] = tempLine.trim().split("\\s+");
                                    if (endString[endString.length - 1].equalsIgnoreCase("end")) {
     ...
     * path
     *
     * @param sourceFile      The path where the source file is located
     * @param destinationFile The path where the copied source file is to be stored
     */
