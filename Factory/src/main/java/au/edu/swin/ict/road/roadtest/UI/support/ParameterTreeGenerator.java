...
public class ParameterTreeGenerator extends JTree implements ActionListener {
     ...
     */
    public ParameterTreeGenerator(ActionListener obj, DefaultMutableTreeNode parent, Map<String, List<Content>> param) {
        super(parent);
        this.obj = obj;
            ...
            if (!currentNode.isRoot()) {
                ...
                if (ae.getActionCommand().equals("insert")) {
                    /*if (c.isArray()) {
                             if (c.isPrimitive())
                             {
                                 Content newElement = generateArrayElement(c);
                                 node = new DefaultMutableTreeNode(newElement);
                                 //c.getMapValues().put(newElement.getName(), newElement);
                                 currentNode.add(node);
                                 ((DefaultTreeModel) this.getModel()).nodeStructureChanged((TreeNode) currentNode);
                             } else if (!c.isPrimitive()) {
                                 Content newElement = generateArrayElement(c);
                                 node = new DefaultMutableTreeNode(newElement);

                                 //try to put this create node function in generateArrayElement
                                 //code and also replace c with newElement
                                 createNode(node, c.getMapValues(),true);
                                 currentNode.add(node);
                                 ((DefaultTreeModel) this.getModel()).nodeStructureChanged((TreeNode) currentNode);
                             }
                         }*/
                ...
                if (ae.getActionCommand().equals("set")) {
                    if (c.isArray() || !c.isPrimitive()) {
                        ...
                        String input = JOptionPane.showInputDialog(null, "Set Value:", "Set", JOptionPane.WARNING_MESSAGE);
                        if (input != null) {
                            c.setValue(input);
                        }
                ...
                if (ae.getActionCommand().equals("get")) {
                    if (c.isArray() || !c.isPrimitive()) {
                        ...
                        if (c.getValue() != null)
                            ...
                            JOptionPane.showMessageDialog(null, "Value not set");
                    }
                ...
                if (ae.getActionCommand().equals("remove")) {
                    ...
                    //this function is commented as currently we dont have complex object or array
                    /*if (!node.isRoot()) {
                             Content nodeC = (Content) currentNode.getUserObject();
                             Content par = (Content) node.getUserObject();
                             if (par.isArray()) {
                                 // nodeC.setMapValues(null);
                                 // declare an integer to hold the  selected  nodes  index
                                 // remove any children of selected node
                                 // remove the selected node,retain its siblings
                                 int nodeIndex = node.getIndex(currentNode);
    ...
    /**
     * This function will generate arrayelement when we try to insert an element to array or complex object
     *
     * @param c <code>Content</code> Parent element object
     * @return <code>Content</code> new element that will be based on parent element
     */
        ...
        /*if(c.isArray() && c.isPrimitive())
          {
              if(c.getMapValues()==null)
              {
                  ...
                  newC.setName(c.getName()+"["+c.getMapValues().size()+"]");
              c.getMapValues().put(newC.getName(), newC);
          }
          else if(c.isArray() && !c.isPrimitive())
          {

              //here yuou should add create node function from above
              newC.setMapValues(c.getMapValues());
