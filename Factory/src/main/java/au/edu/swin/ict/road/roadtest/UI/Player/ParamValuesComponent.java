...
public class ParamValuesComponent extends JPanel {
    ...
    private void generateRows() {
        ...
        if (this.parameterContent.isPrimitive()) {
            if (!this.parameterContent.isArray()) {
                row = new Object[]{parameterContent.getName(), parameterContent.getType(), ""};
                this.jButtonAdd.setVisible(false);
            } else {
                row = new Object[]{parameterContent.getName() + "[" + modelJTableValues.getRowCount() + "]", parameterContent.getType(), ""};
    ...
    protected void jButtonSave_actionPerformed(ActionEvent e) {
        // TODO Auto-generated method stub
        if (JOptionPane.showConfirmDialog(null, "Press OK to confirm all values are set properly.", "Comfirmation", JOptionPane.OK_CANCEL_OPTION) == 0) {
