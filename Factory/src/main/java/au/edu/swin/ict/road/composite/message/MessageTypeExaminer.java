...
public class MessageTypeExaminer implements IMessageExaminer {
    ...
    public void setDestinationRole(String roleId) {
        ...
        /*else
          {
              // Workload allocator
              int i = contractList.get(0).getRoleB().getOutPushQueueSize();

              for(Contract c : contractList)
                  if(c.getRoleB().getOutPushQueueSize() <= i)
                  {
                      i = c.getRoleB().getOutPushQueueSize();
                      destinationContract = c;
                  }
          }		*/
    }
    ...
    /* Leave it empty by default since they are not required for this class to function properly */
