        ...
        try {
            ...
            ClassLoader classLoader =
                    Utils.getClassLoader(configCtx.getAxisConfiguration().
                            getSystemClassLoader(), parentFile, configCtx.getAxisConfiguration().isChildFirstClassLoading());

            Thread.currentThread().setContextClassLoader(classLoader);
            //Deploy the service from java class file
            //File format: CompositeName_RoleName.class
            String className = file.getName();
            String[] compositeRoleName = className.split(ROADConstants.ROAD4WS_SVC_NAME_SEPERATOR);
            ...
            try {
                ...
                String axisServiceName = ROADDeployer.getNoramlizedServiceName(compositeName, roleName);
                if (null != (this.axisConfig.getService(axisServiceName))) {
                    this.axisConfig.removeService(axisServiceName);
                    log.info("[ROAD4WS] Service interface replaced : " + axisServiceName);
                }

                this.axisConfig.addService(axisService);

                String svcDescription = "Role " + roleName + " of composite " + compositeName + "  [ROAD4WS]";

                axisService.setDocumentation(svcDescription);
//                axisService.setServiceDescription(svcDescription);
                log.info("[ROAD4WS] Service added : " + axisService.getName() + " of " + compositeName);
    ...
    private AxisService createAxisServiceUsingAnnogen(String className,
                                                      ClassLoader classLoader, URL serviceLocation) throws ClassNotFoundException,
            InstantiationException, IllegalAccessException, AxisFault {
        //Create a hash map that contains message receiver to create axis service
