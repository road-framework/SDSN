...
public class CustomDefAdaptScaleOut extends CustomDefAdaptationAction {
    ...
    public CustomDefAdaptScaleOut(String commandName, Properties properties) {
        ...
        //this.numOfRoles = Integer.parseInt(super.getProperties().getProperty("numOfRoles"));
    ...
    public boolean adapt(Composite comp) throws AdaptationException {
        ...
        //Wait until the composite is properly deployed in the deployment environment. Query the environment.
        while (true) {
    ...
    private void bindPlayer(Composite comp, IRole role) {
        ...
        String srverAdress = "http://localhost:8080/axis2/services/";//need to get dynamically. As same as the address of the parent organisaiton: TODO comp.getCompositeProperties().get(Constants.COMP_PROP_HOST) ;
        String endPointOfRouterRole = srverAdress + this.roleId + COMPO_PREFIX + "_" + this.roleId + RR_PREFIX;
        endPointOfRouterRole = endPointOfRouterRole.toLowerCase();//Need to convert to a lower case
        ...
        role.unBind();//unbind the existing player. How do we know whether there any incomplete tranasactions?
        //TODO: How to ensure at that point the new composition is deployed and ready to be bound?
//		try {
//			Thread.sleep(10000);
//		} catch (InterruptedException e) {
//			// TODO Auto-generated catch block
...
//		}
     ...
     * <PlayerBindings>
     * </PlayerBindings>
     * <OrganiserBinding>http://localhost:8080/axis2/services/$SMC_NAME$_organizer</OrganiserBinding>
     * <Description>This is the descriptor for the organisation $SMC_NAME$, Generated by Serendip Adaptation Engine</Description>
     * <p/>
     * </tns:SMC>
     */
    public void writeSubComFile(Composite comp) throws AdaptationException {
        OMFactory factory = OMAbstractFactory.getOMFactory();
        OMNamespace roadNS = factory.createOMNamespace("http://www.swin.edu.au/ict/road/smc", "tns");
        //Create SMC root element
        OMElement smcElem = factory.createOMElement("SMC", roadNS);//root

        //Add attributes of SMC
        OMAttribute dataDirAttr = factory.createOMAttribute("dataDir", null, "sample/" + this.smcId + "/data/");
        OMAttribute compositeRuleFileAttr = factory.createOMAttribute("compositeRuleFile", null, "composite.drl");
        OMAttribute routingRuleFile = factory.createOMAttribute("routingRuleFile", null, "routing.drl");
        OMAttribute nameAttr = factory.createOMAttribute("name", null, this.smcId);
        ...
        //We copy the composite.drl and routing.drl rule files. Composite.drl file is the same.
        String dataDirOfNewComposite = comp.getRulesDir() + File.separator + "src/main" + File.separator + ".." + File.separator + ".." + File.separator + this.smcId + File.separator + "data" + File.separator;
        String rulesDirOfNewComposite = dataDirOfNewComposite + File.separator + "rules" + File.separator;
        String transDirOfNewComposite = dataDirOfNewComposite + File.separator + "trans" + File.separator;
        ...
        this.writeRuleFile(comp.getRulesDir() + File.separator + "composite.drl", rulesDirOfNewComposite + "composite.drl");
        if (new File(comp.getRulesDir() + File.separator + this.lblStratergy).isFile()) {
            this.writeRuleFile(comp.getRulesDir() + File.separator + this.lblStratergy, rulesDirOfNewComposite + this.lblStratergy);
        } else {//no such stratergy file. Use default
            this.writeRuleFile(comp.getRulesDir() + File.separator + "routing.drl", "routing.drl");
        }
        ...
        String routerRoleId = this.roleId + RR_PREFIX;//<ER>RR
        {
            ...
            OMAttribute ridAttr = factory.createOMAttribute("id", null, routerRoleId);
            OMAttribute rnameAttr = factory.createOMAttribute("name", null, routerRoleId);
        ...
        for (int i = 0; i < this.newEndPoints.size(); i++) {
            ...
            OMAttribute ridAttr = factory.createOMAttribute("id", null, roleId + i);//<ER>i
            OMAttribute rnameAttr = factory.createOMAttribute("name", null, roleId + i);
            roleElem.addAttribute(ridAttr);
            roleElem.addAttribute(rnameAttr);
            //Add Tasks
        ...
        for (int i = 0; i < this.newEndPoints.size(); i++) {
            String contractId = routerRoleId + "_" + roleId + i;//contractId = <ER>RR_<ER>i
            OMElement contractElem = factory.createOMElement("Contract", null);
            OMAttribute cidAttr = factory.createOMAttribute("id", null, contractId);
            OMAttribute ctypeAttr = factory.createOMAttribute("type", null, "permissive");
            OMAttribute ruleFileAttr = factory.createOMAttribute("ruleFile", null, contractId + ".drl");//TODO: Create drl file
            contractElem.addAttribute(cidAttr);
            contractElem.addAttribute(ctypeAttr);
            contractElem.addAttribute(ruleFileAttr);
            //Create the default contract rule file for the new contract. It contains empty rules
            this.writeRuleFile(comp.getRulesDir() + File.separator + DEFRULEFILE, rulesDirOfNewComposite + contractId + ".drl");
            //Copy the default rule file so that subcomposites too can expand.
            this.writeRuleFile(comp.getRulesDir() + File.separator + DEFRULEFILE, rulesDirOfNewComposite + File.separator + DEFRULEFILE);

            {//TODO: Get terms from the contracts of role=RID and add them to this contract.
                ...
                for (Map.Entry<String, Contract> entry : contracts.entrySet()) {
                    ...
                    boolean routerIsRoleA = true;//To switch the operation direction
                    ...
                    for (Term term : terms) {
                        ...
                        OMElement termElem = factory.createOMElement("Term", null);
                        termsElem.addChild(termElem);
                        OMAttribute termIdAttr = factory.createOMAttribute("id", null, term.getId());
                        OMAttribute termNameAttr = factory.createOMAttribute("name", null, term.getName());
                        termElem.addAttribute(termIdAttr);
                        termElem.addAttribute(termNameAttr);
                        OMElement termDirElem = factory.createOMElement("Direction", null);
                        ...
                        if (null != term.getOperation()) {
                            ...
                            OMElement operationElem = factory.createOMElement("Operation", null);
                            termElem.addChild(operationElem);
                            OMAttribute operationNameAttr = factory.createOMAttribute("name", null, term.getOperation().getName());
                            operationElem.addAttribute(operationNameAttr);
                            //Add <Parameters>
                            OMElement paramsElem = factory.createOMElement("Parameters", null);
                            operationElem.addChild(paramsElem);
                            for (Parameter param : term.getOperation().getParameters()) {
                                //Add <Parameter>
                                OMElement paramElem = factory.createOMElement("Parameter", null);
                                paramsElem.addChild(paramElem);
                                OMElement paramTypeElem = factory.createOMElement("Type", null);
                                paramTypeElem.setText(param.getType());
                                paramElem.addChild(paramTypeElem);
                                OMElement paramNameElem = factory.createOMElement("Name", null);
                                paramNameElem.setText(param.getName());
                                paramElem.addChild(paramNameElem);
                            }
                            //Add <Return>String</Return>
                            OMElement returnElem = factory.createOMElement("Return", null);
                            operationElem.addChild(returnElem);
                            returnElem.setText(term.getOperation().getReturnType());
                        }
                    }
                }
            }
            ...
            roleAElem.setText(routerRoleId);//IMPORTANT:Router role is always RoleA
        ...
        for (int i = 0; i < this.newEndPoints.size(); i++) {
            ...
            OMAttribute pbidAttr = factory.createOMAttribute("id", null, roleId + i + "pb");//<role_id>pb
        ...
        descrElem.setText("This is the descriptor for the organisation " + this.smcId + ", Generated by Serendip Adaptation Tool");
        smcElem.addChild(orgpbElem);
        ...
        try {
            String decrFilPath = System.getenv("AXIS2_HOME") + File.separatorChar + Constants.SERENDIP_COMPOSITE_DIR + File.separatorChar + this.smcId + Constants.SERENDIP_COMPOSITION_FILE_EXT;
    ...
    private void writeRuleFile(String srcFileName, String tgtFileName) throws AdaptationException {
        //	File srcFile = new File(comp.getRulesDir()+File.separator+DEFRULEFILE);
        try {
