 ...
 */
public abstract class ROADDeployer implements Deployer, CompositeAddRoleListener,
        CompositeRemoveRoleListener, CompositeUpdateRoleListener {

    protected static final Logger log = Logger.getLogger(ROADDeployer.class.getName());
    ...
    protected MessagePusher messagePusher; // One instance for all.
    ...
    public void init(ConfigurationContext configurationContext) {
        ...
        this.messagePusher = new MessagePusher(configurationContext, "AXIS2PushListener");
    ...
    public void deploy(DeploymentFileData deploymentFileData)
        ...
        try {
            if (log.isInfoEnabled()) {
                log.info("Loading SMC from " + file.getAbsolutePath());
            }
        ...
        // Set the operational manager service
        OperationalManagerDeployer operationalManagerDeployer = new OperationalManagerDeployer(this.configCtx, composite);
        try {
        ...
        try {
            ...
            if (null != this.axisConfig.getParameter(composite.getName())) {
                ...
                if (log.isInfoEnabled()) {
                    log.info("Adding composite to the axis config "
                            + composite.getName());
                }
    ...
    private void startListeningToCompositeChanges(Composite composite) {
        composite.addCompositeAddRoleListener(this);
        composite.addCompoisteRemovedRoleListener(this);
        composite.addCompositeUpdateRoleListener(this);
    }
    ...
    private void stopListeningToCompositeChanges(Composite composite) {
        composite.removeCompositeAddRoleListener(this);
        composite.removeCompositeRoleListener(this);
        composite.removeCompositeUpdateRoleListener(this);
    }
    ...
    public void undeploy(String fileName) throws DeploymentException {
        ...
        // Get the composite operational manager and remove the services from axis2 engine
        String operationalManagerName = compositeName
                + ROADConstants.ROAD4WS_SVC_NAME_SEPERATOR
                + ROADConstants.ROAD4WS_OPMANAGER_NAME;
        operationalManagerName = operationalManagerName.toLowerCase();
        try {
            AxisServiceGroup serviceGroup = configCtx.getAxisConfiguration()
                    .removeServiceGroup(operationalManagerName);
            configCtx.removeServiceGroupContext(serviceGroup);
            log.info(Messages.getMessage(DeploymentErrorMsgs.SERVICE_REMOVED,
                    fileName));
            log.info("Removed operational manager proxy for composition " + compositeName);
        } catch (AxisFault axisFault) {
            // May be a faulty service
            log.info(Messages.getMessage(
                    DeploymentErrorMsgs.FAULTY_SERVICE_REMOVAL,
        ...
        for (Composite c : compositeList) {
            if (compositeName.equalsIgnoreCase(c.getName())) {
                ...
                for (String roleID : c.getRoleMap().keySet()) {
                    String serviceName = getNoramlizedServiceName(compositeName, roleID);
                    try {
                        configCtx.getAxisConfiguration().removeService(serviceName);
                    } catch (AxisFault axisFault) {
                        log.error("Error removing the axis2 service related to te role : " + roleID, axisFault);
                    }
                }
            }
        }
    ...
    private HashMap getMsgRcvrMap() {
        ...
        MessageReceiver messageReceiver =
                new au.swin.ict.research.cs3.road.road4ws.core.mgsrcvr.ROADInOnlyMessageReceiver();
        messageReciverMap.put(WSDL2Constants.MEP_URI_IN_ONLY,
                messageReceiver);

        MessageReceiver inOutmessageReceiver =
                new au.swin.ict.research.cs3.road.road4ws.core.mgsrcvr.ROADInOutMessageReceiver();
