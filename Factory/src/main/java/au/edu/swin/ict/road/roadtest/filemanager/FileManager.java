...
public class FileManager {
    ...
    /**
     * This function lookup in the folder for all the xml files which represents the message content
     * which is read and kept in memory as hashmap of <location,filecontent>
     *
     * @param folderPath <code>String</code> location of folder
     */
        ...
        if (listOfFiles != null) {
            for (int i = 0; i < listOfFiles.length; i++) {
                if (listOfFiles[i].isFile()) {
                    ...
                    fileContents.put(folderPath + File.separator + fileName, readFile(folderPath + File.separator + fileName));
                }
            }
        }
    }
    ...
    /**
     * This function reads the file content for specific file
     * which is done only once when system is loading
     *
     * @param fileName <code>String</code> location of the file
     * @return
     */
    private String readFile(String fileName) {
        try {
            ...
            while ((s = buffR.readLine()) != null) {
                finalString = finalString + s + System.getProperty("line.separator");
            }
            return finalString;
        } catch (IOException e) {
     ...
     *
     * @return <code>HashMap</code> map containing file location as keys and content of file as values
     */
    ...
    public String[] getFileLocations() {
        String[] keys = (String[]) (fileContents.keySet().toArray(new String[fileContents.size()]));
        return keys;
    }
     ...
     *
     * @param fileKey <code>String</code> key of file location specified in hashmap
     ...
     *
     * @param linesToWrite <code>String</code> lines that needs to be saved in file
