...
/**
 * This is the new implementation for the event pattern recognizer.
 * I have used my own grammar to parse event patterns.
 * The advantages of this method compared to previuos (jboolexpression library)
 * 1. We can support XOR connector
 * 2. We do not need [] brackets for event ids
 *
 * @author Malinda
 */
public class EventPatternRecognizer implements EventPatternParserTokenTypes {
    ...
    public static boolean evaluate(AST ast, String pId, final EventCloud eCloud) throws Exception {
        ...
        switch (ast.getType()) {
            case EventPatternRecognizer.OR:
                ...
                if (orSib == null) {
                    ...
                    return evaluate(orChild, pId, eCloud) || evaluate(orSib, pId, eCloud); // x || y
                }
            ...
            case EventPatternRecognizer.AND:
                ...
                if (andSib == null) {
                    ...
                    return evaluate(andChild, pId, eCloud) && evaluate(andSib, pId, eCloud);
                }
                // break;
            case EventPatternRecognizer.XOR:
                ...
                if (xorSib == null) {
                    ...
                    return evaluate(xorChild, pId, eCloud) ^ evaluate(xorSib, pId, eCloud);
                }
                // break;
            case EventPatternRecognizer.WORD:
                String eventId = ast.getText().trim();
                return eCloud.isEventRecorded(eventId, pId);
            // break;
    ...
    /**
     * Checks of the given event pattern is matched or not,
     ...
     */
    public static boolean isPatternMatched(String eventPattern, String pId, final EventCloud eCloud) throws SerendipException {
        AST ast = null;
        try {
            ast = patternToAST(eventPattern);
            if (null == ast) {
                throw new SerendipException("Cannot parse the expression. " + eventPattern);
            }
            return evaluate(ast, pId, eCloud);
        } catch (Exception e) {
            // TODO Auto-generated catch block
            throw new SerendipException("Pattern matching failed. " + e.getMessage());
        }
    }
    ...
    public static void evaluateExpression(String expression) throws RecognitionException, TokenStreamException, Exception {
        if (log.isDebugEnabled()) {
            log.debug(expression + "=" + evaluateBool(patternToAST(expression)));
        }
    }

    public static void main(String[] args) throws RecognitionException, TokenStreamException, Exception {
        String[] ops = {"^", "|", "*"};
        for (String op : ops) {
