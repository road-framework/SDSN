...
public class DefaultROADDeployer extends ROADDeployer {
    ...
    protected void deployANewRole(IRole role) throws AxisFault {
        ...
        String axisServiceName = getNoramlizedServiceName(role
                .getComposite().getName(), role.getName());
        //We create a service
        AxisService service = new AxisService(axisServiceName);
        ServiceWSDLBuilder serviceWSDLBuilder = new ServiceWSDLBuilder(service, role);
        ...
        for (Operation op : role.getProvidedOperationsList()) {
            //We create a new Axis operation depending on sync/async of operation
            AxisOperation axOp = null;
            if (op.getReturnType() != null && !"void".equals(op.getReturnType())) {//This operation is in-out (there is a return type)
                axOp = new InOutAxisOperation(new QName(op.getName()));
                axOp.setMessageExchangePattern(WSDL2Constants.MEP_URI_IN_OUT);
                axOp.setMessageReceiver(
        ...
        //Push listener need to be registered irrespective of whether there are operations or not.
        ...
        service.addMessageReceiver(WSDL2Constants.MEP_URI_IN_ONLY, messageReceiver);
        service.addMessageReceiver(WSDL2Constants.MEP_URI_IN_OUT, inOutmessageReceiver);
        service.addMessageReceiver(WSDL2Constants.MEP_URI_ROBUST_IN_ONLY, inOutmessageReceiver);
    ...
    private AxisService addAxisServiceInformation(AxisService service) throws AxisFault {
