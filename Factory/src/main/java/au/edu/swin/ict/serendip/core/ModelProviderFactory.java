...
public class ModelProviderFactory {
    ...
    public ModelProviderFactory(SerendipEngine engine) {
        ...
        if (getProperty(Constants.SERENDIP_ROMEO_EXE_PROP) != null) {
            this.verifs = new SerendipVerification[]{new PNVerification()};
        } else {
    ...
    public String getMatchingDefForCoS(String cos) {
        ...
        for (ProcessDefinitionsType pDefType : processDefs) {
            ...
            List<ProcessDefinitionType> processDefinitions = pDefType.getProcess();
    ...
    public boolean verifyPD(String defId) throws SerendipVerificationException {
        ...
        for (SerendipVerification verif : this.verifs) {
            VerificationResult result = verif.verifyProcessDef(this, defId);
            if (!result.isValid()) {
                return false;
            }
        }
    ...
    public ProcessInstance getNewProcessInstance(Classifier classifier, String defId)
            throws SerendipException {
        ProcessInstance pi = new ProcessInstance(this.engine, defId, classifier);

        // Create behavior terms
        List<String> btIdList = this.engine.getComposition().getAllBehaviorTermIdsForPD(defId);
        ...
        for (String btId : btIdList) {
            ...
            bt.subscribeMonitorTo(this.engine.getEventCloud());
    ...
    public ProcessInstance getNewProcessInstanceV2(Classifier classifier, String defId) {
        return new ProcessInstance(this.engine, defId, classifier);
    }
    ...
    public BehaviorView getBehaviorView(String btId) throws SerendipException {
        ...
        CollaborationUnitType btType = this.engine.getComposition().getCollaborationUnitTypeById(btId);
        BehaviorTerm bt = new BehaviorTerm(btType, this.engine.getComposition().getComposite());
     ...
     *
     * @param pDefId
     * @param pid    (Optional if u need a specific instance
     * @return
     * @throws SerendipException
     */
    public ProcessView getProcessView(String pDefId, String pid)
        ...
        if (null == pid) {
            ...
            pi = this.getNewProcessInstance(null, pDefId);
        } else {
            pi = this.engine.getProcessInstanceByInsId(pid);
        }
