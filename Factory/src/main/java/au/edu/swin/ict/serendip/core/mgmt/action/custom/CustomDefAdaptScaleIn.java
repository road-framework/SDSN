...
//			}
        ...
        if ((comp.getRoleMap().size() - 2) <= roleArrList.size()) {
            //In this case we left with just one role. We need to remove the complete composite and allocate the remaining player to the parent composite.
            System.out.println("TODO:Remove ExpOrg");
        } else {
            //We need to keep the composite but remove a subset of roles
            for (Role role : roleArrList) {
                //We remove adjoining contracts. Well.. in reality there is only one for a scaled out composite
