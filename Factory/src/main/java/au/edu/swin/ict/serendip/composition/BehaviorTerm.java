...
import au.edu.swin.ict.road.common.BehaviorUnitManagementState;
import au.edu.swin.ict.road.composite.Composite;
import au.edu.swin.ict.road.regulator.BehaviorMonitor;
import au.edu.swin.ict.road.regulator.BehaviorMonitorInstance;
import au.edu.swin.ict.road.xml.bindings.CollaborationUnitType;
import au.edu.swin.ict.road.xml.bindings.MonitorType;
...
public class BehaviorTerm {
     ...
     * @param extendsFrom
     */
    public BehaviorTerm(String id, List<Task> tasksVec, String extendsFrom,
                        boolean isBastract) {
        this.id = id;
     ...
     */
    public BehaviorTerm(CollaborationUnitType btType, ProcessInstance pi,
    ...
    public BehaviorTerm(CollaborationUnitType btType, Composite composite) throws SerendipException {
        super();
//		if(null == btType){
...
//		}
    ...
    private void init(CollaborationUnitType btType, Composite composite) throws SerendipException {
        if (null == btType) {
            throw new SerendipException("Invalid behavior term " + btType);
        }
        this.id = btType.getId();
     ...
     * specializes behavior term B'.
     * B -> B'(parent)
     * Rule 1. All tasks specified in B' but not specified in B are inherited
     * from B'. All the tasks specified in B but
     * not specified in B' are recognised as newly specified tasks of B.
     * <p/>
     * Rule 2. If there are tasks common to both B and B', the tasks of B overrides
     * tasks of B'. (See next rule)
     * <p/>
     * Rule 3. When a task T of B (B.T) overrides a task T of B' (B'.T)
     * 3.a. All attributes specified in Task B'.T but not specified in
     * B.T are inherited by T.
     * 3.b. If there are attributes common to both B.T and B'.T,
     * the attributes of B.T are considered.
     * Rule 4. All constraints specified in B' are inherited by B.
     * Rule 5. Constraints cannot be overridden, i.e. a constraints in
     * B cannot use an identifier of a constraint of B' to
     * specify a new constraint.
     * <p/>
     * Other rules:
     * Rule 6. All the attributes of tasks of a concrete behavior
     * term need to be fully specified. Rule 7. If attributes
     * of tasks are not fully specified (due to lack of knowledge),
     * the behavior term should be explicitly declared abstract.
     *
     * @throws SerendipException
     */
    private void supportInheritance() throws SerendipException {
        if (null != this.extendsFrom) {
            ...
            CollaborationUnitType parentType = CompositionUtil.getCollaborationUnitType(this.extendsFrom, this.composite);
            //If invalid behavior term, throw an exception
            if (null == parentType) {
                throw new SerendipException("No such parent behavior term for id=" + this.extendsFrom);
            }
            //Model the parent behavior term
            BehaviorTerm parentBt = new BehaviorTerm(parentType, this.composite);
            //Get the tasks of parents
            List<Task> parentTasks = parentBt.getAllTasks();
            for (Task t : parentTasks) {
                //Check if there are any overrides
                //If so ignore those from the parent and preserve the child's
                String taskId = t.getId();
                if (!this.containsTask(taskId)) {//If child does not have the task t
                    if (null != this.pi) {
                        t.setPi(pi); // TODO INDIKA
                        t.setClassifier(this.pi.getClassifier());//We need to set the pid
                    }
                    this.tasksVec.add(t);//We add the inherited task from the parent
                } else {//We do have such a task
                    //And, we might need to check whether the child task needs to inherit some properties form parent's counterpart
                    Task childTask = this.getTask(taskId);//Child task
                    //if(childTask.isOverride()){
                    //Now we know the child task is expecting to override the parent
                    //So what are the values in child that are empty?
                    //Get those from the parent
                    if ((null == childTask.getEventPatterns().get(0)) || (
                            childTask.getEventPatterns().get(0).equals(""))) {
    ...
    public void subscribeTasksTo(EventCloud ec) {
        if (null != ec) {
            for (Task t : this.tasksVec) {
                t.setClassifier(this.pi.getClassifier());
                t.subscribeTo(ec);
            }
        }
    }

    public void subscribeMonitorTo(EventCloud ec) {
        String monitorId = null;
        if (btType != null) {
            MonitorType monitorType = btType.getMonitor();
            if (monitorType != null) {
                monitorId = monitorType.getId();
            }
        }
        if (monitorId == null || monitorId.isEmpty()) {
            monitorId = id + "." + "monitor";
        }
        BehaviorMonitor behaviorMonitor = composite.getBehaviorMonitor(monitorId);
        if (null != behaviorMonitor) {
            BehaviorMonitorInstance bmIns = new BehaviorMonitorInstance(behaviorMonitor);
            bmIns.setClassifier(this.pi.getClassifier());
            ec.subscribe(bmIns);
        }
    }
    ...
    throws SerendipException {
        ...
        if (taskRefList.size() < 1) {
            if (log.isInfoEnabled()) {
                log.warn("WARNING: " + btType.getId() + " contains " + taskRefList.size() + " tasks");
            }
        }
        for (TaskRefType tt : taskRefList) {// Iterate thru Task types and
            ...
            if (tt.getId().contains(".")) {
                String[] taskSplit = tt.getId().split("\\.");//Seperate the role from the task id
                roleId = taskSplit[0];
                taskId = taskSplit[1];
            } else {
                taskId = tt.getId();
...
//            task.setPreEPIds(tt.getPreEPIds());
    ...
    public boolean changeEventPattern(String taskId, String newEventPattern) {
        ...
        for (int j = 0; j < this.getTasksVec().size(); j++) {
            ...
            if (task.getId().equals(taskId)) {
                // Task found
                task.addEventPattern(newEventPattern);
                return true;
            }
        }
     ...
     * @return
     */
    public List<Task> getTasksOfRole(String roleId) {
        List<Task> tasksOfRoleVec = new ArrayList<Task>();
        for (int i = 0; i < this.tasksVec.size(); i++) {
            Task t = this.tasksVec.get(i);
